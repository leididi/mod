...
def gen_csv(src_data,dst_data,basedir=None):
    ...
    if not os.path.exists(csvpath):
        os.makedirs(csvpath)
    with open(jsonpath+os.sep+fname+"_src.json","w+",encoding='utf-8') as f:
        json.dump(src_data,f,ensure_ascii=False,indent=4)
    with open(jsonpath+os.sep+fname+'_dst.json','w+',encoding='utf-8') as f:
        json.dump(dst_data,f,ensure_ascii=False,indent=4)
    with open(jsonpath+os.sep+fname+"_src.json",'r',encoding='utf-8') as f:
        srclist=f.readlines()
    with open(jsonpath+os.sep+fname+'_dst.json','r',encoding='utf-8') as f:
        dstlist=f.readlines()
    ...
    with open(csvpath+os.sep+fname+'.csv','w+',newline='',encoding='utf-8') as f:
